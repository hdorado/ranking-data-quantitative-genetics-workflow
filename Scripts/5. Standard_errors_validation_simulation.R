...
# Validation standard error of the worth values estimates
# Hugo Dorado
# 14-11-2024
...
repetition <- expand.grid(iter=1:100,rep=1:30 ,n.rep.genotype.env=c(3,30,100),
            sigma.genotype=c(0.05,0.8,2),sigma.plot = c(0.4,0.9,5.0)
            )
...
rank_experiment <- function(w){
  ...
  mu.gs <- rnorm(n = 20,mean=mu.genotype,sd=sigma_g)
  ...
  saveRDS(outputs,paste0('standard_error_val/output_',iter,'-',rep,'-',sigma_g,'-',sigma.plot,'-',n.rep.genotype.env,'.rds'))
  
  outputs
}

outputs <- mclapply(seq(nrow(repetition)), rank_experiment,mc.cores = 60)

# runned at Standard_errors
